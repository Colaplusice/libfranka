...
int main(int argc, char** argv) {
  ...
  try {
    ...
    double time = 0.0;
    static std::vector<std::array<double, 7>> all_joint_values = read_joint_position_from_file("/home/ubuntu/Documents/velocity.txt");
    robot.control(
        [=, &time](const franka::RobotState&, franka::Duration period) -> franka::JointVelocities {
          ...
          franka::JointVelocities velocities = current_value;
          if (time >= 2 * time_max) {
